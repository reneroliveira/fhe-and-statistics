...
[Ring Learning with Errors - RLWE] Set integers $n,q$, with $n$ being a power of two, and $2\leq q \leq \poly(n)$. Let $f(x)=x^n+1$ (the $2n$-th cyclotomic polynomial), define $R=\Z[X]/( f(X))$, a distribution $\chi$ over $R$ and $R_q=R/(q) =\Z_q[X]/(f(X))$ which can be represented by polynomials with integer coefficients wrapped by $q$ and with degree up to $n-1$.
...
The message $m$ of the HEAAN scheme will live in the plaintext of the cyclotomic ring $R_{q}=\Z_q[X]/(X^N+1)$ with $N$ being a power of tho; the ciphertext will be tuples in $R_q\times R_q$. This can be quite contradicting to the proposal of the scheme for supporting complex numbers, but this can be reached using a clever encoding/decoding procedure that can map a complex vector $z\in \mathbb C^{N/2}$ to a polynomial $m\in R_q$, and vice-versa. This allows us to perform operations in a SIMD\footnote{Single instruction, multiple data}, saving computational costs. 
...
The scheme requires a process called \textit{relinearization} after each multiplication. Directly multiplying two ciphertexts $\boldsymbol c =(c_1,c_2)$ and $\boldsymbol c'=(c'_1,c'_2)$ yields a vector in $(d_0,d_1,d_2)\in R_q^3$ where:
\begin{align*}
    d_0 & = c_1\cdot c'_1\\
