...
In this chapter, we shall discuss the algebraic theoretical background that grounds cryptography schemes. Section \ref{sec:basics} defines basic algebraic structures such as groups and rings. Section \ref{sec:homo} goes through homomorphism properties and quotient rings, two important concepts. Section \ref{sec:cyc} explains cyclotomic polynomials, a special type of polynomial that will be used in the next section and chapter. Section \ref{sec:lattices} defines lattices, an algebraic structure that has interesting computationally hard problems that will guarantee the security of our encryption schemes.
...
\begin{align*}
    ...
    \varphi(1_R)&=1_S
\end{align*}
Similarly, we can define group homomorphisms that will preserve their unique underlying operation. $\varphi(0_R)=0_S$ is a consequence of group properties\footnote{Here $0_R$ denotes the identity element of $R$ when viewed as an additive group} \cite{omn}.
\end{definition}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $\ker\varphi$ is closed under (additive) inverse: If $a\in\ker\varphi$, $\varphi(a)=0$ then $0=\varphi(0)=\varphi(a+(-a))=\varphi(a)+\varphi(-a)=\varphi(-a)$. So $\phi(-a)=0$ impling $-a\in\ker\varphi$
    \end{itemize}
\end{proof}
\begin{definition}[Isomorphism]
    A map $\varphi:R\to S$ is called an isomorphism if it is a bijective homomorphism. In this case, we denote $R\cong S$.
\end{definition}
A famous example of group isomorphism is \begin{align*}\varphi&:\C\to\mathbb{R}^2\\\varphi(a+bi)&=(a,b),\end{align*}
...
\begin{definition}[Cosets]
    ...
    $$\lambda_g(H)=\{g\cdot h|~h\in H\},$$
    is called a \textit{coset} of $H$.
    
    We denote the set of all cosets of $H$ by:
$$G/H\stackrel{\text{def}}{=}\{g\cdot H|g\in G\}$$
\end{definition}
...
$$R/I=\{I+r|r\in R\}$$
For example: Take the ring of integers $\Z$ and the multiples of three ideal $I=(3)$. The cosets of $I$ are translations the translations of $I=\{\ldots,-3,0,3,6,\ldots\}$:
\begin{align*}
...
Let $\Z[X]$ be the polynomials with integer coefficients and $f(x)= x^n+1$, a monic polynomial with degree $n$. Define the homomorphism $\varphi:\Z[X]\to\Z[X]$, that take a polynomial $p(x)\in\Z[X]$ and returns $r(x)$, the remainder of the polynomial division $p(x)/f(x)$, using grade-school division algorithm. $\ker\varphi $ is the set of polynomials with remainder equal $0$, i.e., the set of multiples of $x^n+1$ in $\Z[X]$, which is the ideal generated by $f$ (Prop. \ref{prop:kernel}). The Homomorphism Theorem affirms that $\Z[X]/(f(x))\cong\varphi(\Z[X])$, so we can represent the set of cosets of $(x^n+1)$ simply as the image of $\varphi$: the set of remainders in the division by $f(x)$, which can be viewed as the integer polynomials with degree less than $n$. This will be a central ring in the encryption schemes we shall further define.
...
\begin{alineas}
    \item Mini-batch gradient descent: The gradient is calculated over a subset of the data instead of the whole dataset. Notice that gradient equation \ref{eq:gradient}, requires $n$ multiplications between sigmoid result and $\bd z_i$, operation that can be expansive or even impossible to do homomorphically for a large $n$. Miini-batch procedure can also take advantage of the message encoding process described in previous chapter.
...
\end{align*} where $\gamma$ and $\eta$ are parameters that must be set by the user.
 \item Polynomial approzimation for sigmoid: It's not possible to evaluate $\exp(-x)$ homomorphically, so the authors propose the use of a polynomial approximation for the sigmoid function, fitted using least squares minimization. The degree $3$ approximation, that will be used in our tests, is $0.5+0.15x+-0.0015x^3$, and it's the polynomial has the least square distante to the actual sigmoid in the interval $[-8,8]$ (Figure \ref{fig:sigmoid}). It's desirable to have a low degree polynomial in order to save multiplications, and a cubic function provides a decent estimate.
\end{alineas}
\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.3]{files/figures/sigmoid.png}
    \caption{Sigmoid polynomial approximation}
    \label{fig:sigmoid}
\end{figure}

Let $\boldsymbol Z$ be the $n\times(d+1)$ matrix where the columns are $\bd z_i=\bd x'_i\cdot y'_i$. We'll encrypt it as blocks benefiting from CKKS encoding technique. Let's denote $\bd Z = [\bd y',~\bd x'_1,~\bd x'_2,~\ldots,~x'_d]$ more explicitly:
