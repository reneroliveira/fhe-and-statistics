...
With the SIMD mask multiplication, the total multiplicative cost of the batch inner product is $2$.

Now we have all tools to redefine the loop of Algorithm \ref{alg:alg1} to its vectorized form and compatible with HEAAN homomorphic operations. For a fixed iteration $i$, $\Z_j=Z_{i,j}$ as in 

\begin{algorithm}[!htb]
    \caption{Vectorized weights update \label{alg:alg1}}
    \begin{algorithmic}[1]
        \REQUIRE{$Z_j,W_j,V_j$}
\ENSURE{Weight vectors $\bd w,\bd v\in\mathbb{R}^{d+1}$}
 \STATE Initializes $\bd w,\bd v\leftarrow \bd 0$;
